...
from data.custom_dataset import ImageEmbeddingDataset, expand_urls
...
class DataModuleCustom(LightningDataModule):
    ...
    def add_data_specific_args(parent_args):
        ...
        parser.add_argument('--test_prompts', type=str, default="./test_prompts.json")
        parser.add_argument('--test_repeat', default=2, type=int)
        parser.add_argument('--merge_cat', default=False, action="store_true")
        parser.add_argument('--shuffle_cat', default=False, action="store_true")
        parser.add_argument('--phrase_dropout_rate', type=float, default=0.3, help="Drop phrases with this probability during training")

        parser.add_argument("--resolution", type=int, default=512, help="Image resolution for input resizing")
        parser.add_argument("--center_crop", action="store_true", default=False, help="Use center crop before resizing")
        return parent_args

    def __init__(self, args, tokenizer, collate_fn=None, use_worker_init_fn=None):
        super().__init__()
        splits = {
    ...
    def prepare_data(self):
        ...
        assert num_train + num_test + num_val == len(all_urls)
        self.train_urls, self.test_urls, self.val_urls = random_split(
            all_urls, [num_train, num_test, num_val]
        )
    ...
    def setup(self, stage=None):
        if 'train' in self.datasets:
            self.datasets['train'] = ImageEmbeddingDataset(
                ...
                drop_phrases=True,  # ← 启用短语dropout
                phrase_dropout_rate=self.args.phrase_dropout_rate  # ← 控制概率
            )
